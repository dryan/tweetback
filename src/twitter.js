...
class Twitter {
  ...
  isSearchMatch(tweet, needle, caseSensitive, before) {
    ...
    return (
      needles.filter(
        (needle) =>
          !!haystack.match(
            new RegExp("\\b" + needle + "\\b", "g" + (!caseSensitive ? "i" : ""))
          )
      ).length > 0
    );
  }
      ...
      for (let media of tweet.extended_entities.media) {
        if (media.type === "photo") {
          ...
          try {
            ...
            imgHtml = `<img src="${imgRef.url}" width="${imgRef.width}" height="${
              imgRef.height
            }" alt="${
              media.alt_text ||
  ...
  async renderTweet(tweet, options = {}) {
    ...
    let shareCount =
      parseInt(tweet.retweet_count, 10) + (tweet.quote_count ? tweet.quote_count : 0);
    ...
    return `<li id="${tweet.id_str}" class="tweet${
      ...
      this.isReply(tweet)
        ? `<a href="${
          ...
          }" class="tweet-pretext">…in reply to @${tweet.in_reply_to_screen_name}</a>`
        : ""
    }
  ...
  getMostPopularTweets(tweets, limit = 15, forYear) {
    function sortByShareCount(a, b) {
      let shareCountA =
        (parseInt(a.retweet_count, 10) || 0) + (parseInt(a.quote_count, 10) || 0);
      let shareCountB =
        (parseInt(b.retweet_count, 10) || 0) + (parseInt(b.quote_count, 10) || 0);
      return shareCountB - shareCountA;
    }

    let topTweets = tweets.filter((tweet) => {
      return (
        !this.isRetweet(tweet) && (!forYear || forYear == tweet.date.getFullYear())
      );
    });
    let top = new Set();
