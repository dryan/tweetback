...
class Index extends Twitter {
  ...
  async render(data) {
    ...
    let last12MonthsTweets = tweets.filter(
      (tweet) => tweet.date - new Date(Date.now() - 1000 * 60 * 60 * 24 * 365) > 0
    );
    ...
    let retweetsEarnedCount = tweets
      ...
      .reduce(
        (accumulator, tweet) => accumulator + parseInt(tweet.retweet_count, 10),
        0
      );
    let likesEarnedCount = tweets
      .filter((tweet) => !this.isRetweet(tweet))
      .reduce(
        (accumulator, tweet) => accumulator + parseInt(tweet.favorite_count, 10),
        0
      );
    ...
    let mostPopularTweetsHtml = await Promise.all(
      this.getMostPopularTweets(tweets)
        .slice(0, 6)
        .map((tweet) => this.renderTweet(tweet, { showPopularity: true }))
    );
    ...
    return `
		<h2 class="tweets-primary-count">
			<span class="tweets-primary-count-num">${this.renderNumber(tweetCount)}</span> tweet${
      tweetCount !== 1 ? "s" : ""
    }
		</h2>
		...
		<div>
			<h2><a href="/tweets/recent/">Recent:</a></h2>
		...
		<div>
			<h2><a href="/tweets/popular/">Popular:</a></h2>
      ...
      retweetCount
		...
		<div class="lo" style="--lo-stackpoint: 20em">
            ...
            .map(
              (user) =>
                `<li><a href="${twitterLink(`https://twitter.com/${user.username}`)}">${
                  user.username
                }</a> ${user.count} retweet${user.count != 1 ? "s" : ""}</li>`
            )
            .join("")}
				</ol>
			</div>
		...
		<div class="lo" style="--lo-stackpoint: 20em">
            ...
            .map(
              (user) =>
                `<li><a href="${twitterLink(`https://twitter.com/${user.username}`)}">${
                  user.username
                }</a> ${user.count} repl${user.count != 1 ? "ies" : "y"}</li>`
            )
            .join("")}
				</ol>
			</div>
    ...
    )} times (${this.renderPercentage(mentionNotReplyCount, tweetCount)})</h3>
      ...
      noRetweetsTweetCount
        ...
        .map(
          (hash) =>
            `<li><code>${hash.tag}</code> used ${hash.count} times ${
              hash.count > 1 && hash.count > hash.tweets.length
                ? `on ${hash.tweets.length} tweet${hash.tweets.length !== 1 ? "s" : ""}`
                : ""
            }</li>`
        )
        .join("")}
      ...
      noRetweetsTweetCount
        ...
        .map(
          (swear) =>
            `<li><code>${this.renderSwearWord(swear.word)}</code> used ${
              swear.count
            } times ${
